...
def Get():
	...
	m3u  = 'http://powertv.xyz%3A8080%2Fget.php%3Fusername%3D'+username+'%26password%3D'+password+'%26type%3Dm3u_plus%26output%3Dts'
	epg  = 'http://powertv.xyz%3A8080%2Fxmltv.php%3Fusername%3D'+username+'%26password%3D'+password
	auth = 'http://powertv.xyz:8080/enigma2.php?username='+username+'&password='+password+'&type=get_vod_categories'
	...
	if not auth=="":
		...
		xbmcgui.Dialog().ok('Power TV','[COLOR cornsilk]M3U URL: [/COLOR]%s'%shortm3u,'','[COLOR cornsilk]EPG URL: [/COLOR]%s'%shortepg)
	else:
		return