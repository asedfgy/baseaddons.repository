...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.streamtvsupertuga.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.streamtvsupertuga.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.streamtvsupertuga.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.streamtvsupertuga.artwork)')
...
def moderator():
    ...
    if not infoLabel('Container.PluginName') in netloc:
        infoDialog('Stream TVsupertuga: Unknown screen or plugin', sound=True)
        sys.exit()

def metaFile():
    if condVisibility('System.HasAddon(script.streamtvsupertuga.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.streamtvsupertuga.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
