...
username     = xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('Username')
password     = xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('Password')
...
def start(type):
		...
		if "username" in auth:
			...
			if exp == 'Expired':
				xbmcgui.Dialog().ok(user.name,'Your Account Has Expired! %s'%username,'AJUDA: https://www.facebook.com/TVsupertuga.PT/')
				sys.exit()
			if type=="NEW":
				xbmcgui.Dialog().ok(user.name, 'Welcome To %s, %s'%(user.name,username), 'obrigado por usar o nosso addon', 'team tvsupertuga')
				tvguidesetup()
				addonsettings('ADS2','')
			...
			if not d:
				...
				xbmcaddon.Addon('plugin.video.TVsupertuga').openSettings()
				if type == "NEW":
					type = 'NEW'
				else:
					type = 'NONE'
...
def addonsettings(url,description):
	...
	if   url =="CC":
		...
		if 'ON' in description:
			xbmcaddon.Addon('plugin.video.TVsupertuga').setSetting('vodsource','false')
			xbmc.executebuiltin('Container.Refresh')
		else:
			xbmcaddon.Addon('plugin.video.TVsupertuga').setSetting('vodsource','true')
			xbmc.executebuiltin('Container.Refresh')
	elif url =="LO":
...
def extras():
	...
	if xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('vodsource')=='true':
...
def startupd():
	try:
		if xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('startupd') == '0':raise Exception()
		...
		if xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('startupd') == '2':
			xbmcgui.Dialog().notification('[COLOR ffff0000][B]StreamHub[/B][/COLOR]','Welcome Back, You Have %s Days Left Of Premium Content'%days)
		elif xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('startupd') == '1':
