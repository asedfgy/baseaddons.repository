...
SOURCES = [{"title": "script.module.loganaddon.addon ", "url": "http://bit.ly/29ojshgsgbsfhbfkjbbgjskbgjbsgjjhgbdjvbd6653735r3hfhbf", "fanart": "http://www.technobuffalo.com/wp-content/uploads/2016/02/tim-peake-photos-003.jpg", "genre": "IPTV TELEVISION", "date": "10.17.2015", "credits": "script.module.loganaddon.addon", "thumbnail": "http://i86.servimg.com/u/f86/19/44/91/43/icon10.png"}]
...
Q1RWQmFzZQ0K = base64.b64decode('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0JaRHAzcmto')



def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.script.module.loganaddon.addon Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(script.module.loganaddon.addon,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(script.module.loganaddon.addon,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def TVsupertuga():
    addon_log("TVsupertuga")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(TVsupertuga,New source added.,5000,"+icon+")")
        if not url is None:
...
def WizHDMenu(url,iconimage,fanart):
	addDir('TVsupertuga Playlisted',url,1,iconimage,fanart,'','','',None,'source')
...
SOURCES = [{"title": "TVsupertuga ", "url": "http://bit.ly/29ojshgsgbsfhbfkjbbgjskbgjbsgjjhgbdjvbd6653735r3hfhbf", "fanart": "http://www.technobuffalo.com/wp-content/uploads/2016/02/tim-peake-photos-003.jpg", "genre": "IPTV TELEVISION", "date": "10.17.2015", "credits": "TVsupertuga", "thumbnail": "http://i86.servimg.com/u/f86/19/44/91/43/icon10.png"}]
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('TVsupertuga Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('TVsupertuga Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(TVsupertuga,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(TVsupertuga,Is Finding Your Link - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('TVsupertuga','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('TVsupertuga', 'Do you want to add this file as a source?')
        if ret:
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(TVsupertuga ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(TVsupertuga,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
