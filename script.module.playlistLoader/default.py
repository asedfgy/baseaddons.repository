...
AddonID = 'script.module.playlistLoader'
...
def Categories():
	AddDir("[COLOR dodgerblue][B]{0}[/B][/COLOR]".format(getLocaleString(10001)), "settings" , 20, os.path.join(iconsDir, "NewList.ico"), isFolder=False)
	AddDir("[COLOR green][B][{0}][/B][/COLOR]".format(getLocaleString(10003)), "favorites" ,30 ,os.path.join(iconsDir, "bright_dodgerblue_star.png"))
	cacheList = []
	i = 0
	...
	for item in list:
		...
		AddDir("[COLOR red][{0}][/COLOR]".format(name) ,item["url"].encode("utf-8"), mode, image.encode("utf-8"), logos.encode("utf-8"), index=i, cacheMin=cacheMin)
		i += 1
...
def PlxCategory(url, cache):
	...
	for channel in list[1:]:
		...
		if channel["type"] == 'playlist':
			AddDir("[COLOR red][{0}][/COLOR]".format(name) ,channel["url"].encode("utf-8"), 1, iconimage, background=background.encode("utf-8"))
		else:
...
def ListFavorites():
	AddDir("[COLOR dodgerblue][B]{0}[/B][/COLOR]".format(getLocaleString(10013)), "favorites" ,34 ,os.path.join(iconsDir, "bright_dodgerblue_star.png"), isFolder=False)
