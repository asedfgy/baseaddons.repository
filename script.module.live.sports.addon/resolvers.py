...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.Live Sports Lists-%s]: %s" %(addon_version, string))
...
def table():
    ...
    sp1 = '                                         '
    sp2 = '        '
    addDir('[COLORred][B]'+sp1+'pl'+sp2+'w'+sp2+'d'+sp2+'l'+sp2+'f'+sp2+'a'+sp2+'pts[/B][/COLOR]','','','https://upload.wikimedia.org/wikipedia/commons/9/9b/Liga_NOS_logo.gif',
'','','','','')
    html = requests.get('http://www.livefootball.com/football/portugal/liga-sagres/').content
    match = re.compile('</li><li><a href="/football/portugal/liga-sagres/league-table/away/".+?title="Rank">([^<]*)</td>.+?title="Played">([^<]*)</td>.+?title="Wins">([^<]*)</td>.+?title="Draws">([^<]*)</td>.+?title="Lost">([^<]*)</td>.+?title="Goals For">([^<]*)</td>.+?title="Goals Against">([^<]*)</td>.+?title="Goals Difference">([^<]*)</td>.+?title="Points">([^<]*)</td>(.+?)</table></div>',re.DOTALL).findall(html)            
    #match2 = re.compile('</li><li><a href="/football/portugal/liga-sagres/league-table/away/".+?<td class="ltn">([^<]*)</td><td class="ltg">([^<]*)</td><td class="ltw">([^<]*)</td><td class="ltd">([^<]*)</td><td class="ltl">([^<]*)</td><td class="ltgf">([^<]*)</td><td class="ltga">([^<]*)</td><td class="ltgd">([^<]*)</td><td class="ltp">([^<]*)</td></tr><tr><td class="ltid">([^<]*)</td>',re.DOTALL).findall(str(rest))
    for r,p,w,d,l,f,a,df,pts,rest in match:
        ...
        for iD,team,pl,w,d,l,f,ga,gd,p, in match2:
            pos = iD+team
            team = team.replace('Team &amp; dir','Team').replace('BOAVISTA','BOAVISTA')
            team = team.replace('VITORIA DE GUIMARAES','VITORIA DE GUIMARAES').replace('FC PORTO','FCP').replace('SPORTING','SCP').replace('BENFICA','SLB')
            team = team.replace('CHAVES','CHAVES')
            pl_sp = gap(pl)
            w_sp = gap(w)
            d_sp = gap(d)
            l_sp = gap(l)
            ...
            image = thing[1]
            sp_no = thing[0]
            addDir(team+sp_no+pl+pl_sp+w+w_sp+d+d_sp+l+l_sp+f+f_sp+ga+a_sp+' '+p,'','',image,'','','','','')
...
def Cleaner(team):
    ...
    if '<a href' in team:
        ...
        if  'FC PORTO'      in team:         
            image = 'https://logodownload.org/wp-content/uploads/2017/03/porto-fc-logo-escudo-3.png'
            gap =    '                            '
        elif 'BENFICA' in team:
            image = 'https://upload.wikimedia.org/wikipedia/en/thumb/2/22/Benfica_Luanda_logo.svg/500px-Benfica_Luanda_logo.svg.png'
            gap = '                          '
        elif 'SPORTING'     in team:
            image = 'https://upload.wikimedia.org/wikipedia/en/thumb/e/e1/Sporting_Clube_de_Portugal_%28Logo%29.svg/763px-Sporting_Clube_de_Portugal_%28Logo%29.svg.png'
            gap = '                '
        elif 'BRAGA' in team:
            image = 'https://upload.wikimedia.org/wikipedia/en/thumb/7/79/S.C._Braga_logo.svg/410px-S.C._Braga_logo.svg.png'
            gap = '                            '
        elif 'MARITIMO' in team:
            image = 'https://upload.wikimedia.org/wikipedia/en/6/67/Club_Sport_Mar%C3%ADtimo.png'
            gap = '                            '
        elif 'VITORIA DE GUIMARAES' in team:
            image = 'https://upload.wikimedia.org/wikipedia/pt/b/b3/VitoriaGuimaraes.png'
            gap = '                  '
        elif 'RIO AVE' in team:
            image = 'https://upload.wikimedia.org/wikipedia/pt/f/f5/Logo_Rio_Ave.png'
            gap = '                             '
            ...
            gap = '                          '
        elif 'BELENENSES' in team:
            image = 'https://upload.wikimedia.org/wikipedia/pt/e/e1/Logo_Belenenses.png'
            gap = '                            '
            ...
            gap = '                  '
        elif 'PACOS DE FERREIRA' in team:
            image = 'https://upload.wikimedia.org/wikipedia/pt/3/3d/Futebol_Clube_Pa%C3%A7os_de_Ferreira.png'
            gap = '                         '
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Live Sports,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Live Sports,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    if addon.getSetting('Intro_Vid')=='true':
        INTRO_VID = ''
        xbmc.Player().play(INTRO_VID, xbmcgui.ListItem('Live Sports'))
    addon_log("SKindex")
    addDir('[COLOR yellow][B]FAVORITES[/B][/COLOR]','Favorites',4,'https://i62.servimg.com/u/f62/19/44/91/43/favori10.png' ,  FANART,'','','','') 	
    addDir('[COLOR deepskyblue][B]FOOTBALL HIGHLIGHTS[/B][/COLOR]','Football Highlights',33,'https://pbs.twimg.com/profile_images/871207412581126144/LgnctUgA.jpg' ,  FANART,'','','','')
    addDir('[COLOR deepskyblue][B]LIGA NOS TABELA[/B][/COLOR]','Premier-league-table',60,'https://i62.servimg.com/u/f62/19/44/91/43/ligano10.png' ,  FANART,'','','','')
    getData(readme.Readme,'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Live Sports,New source added.,5000,"+icon+")")
        if not url is None:
...
def WizHDMenu(url,iconimage,fanart):
	addDir('Paul\'s Picks',url,1,iconimage,fanart,'','','',None,'source')
...
#hakamac code
    ...
    List=[]
    List.append(readme.Readme)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('Live Sports Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Live Sports,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Live Sports,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Live Sports','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Live Sports', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Live Sports,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            contextMenu.append(('[COLOR red]!!Download Currently Playing!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=21&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))           
            ...
            contextMenu.append(('[COLOR red]!!Download!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=23&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name)))) 
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Live Sports ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Live Sports,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
