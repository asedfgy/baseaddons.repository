...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.skyonplus Lists-%s]: %s" %(addon_version, string))
...
base = "L5kdGp2MIh3L3Fmcv02bj5ibpJWZ0NXYw9yL6MHc0RHa"
tam = len(base)
basedem = base[::-1]
...
def SKindex():
    addon_log("SKindex")
    addDir('[COLOR white][B]DOAÇÕES PARA ADDON [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','100',100,icon,FANART,'','','','')
    addDir('[COLOR white][B]CREDITOS PARA ADDON [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','101',101,icon,FANART,'','','','')
    addDir('-------------------','101',901,icon,FANART,'','','','')
    getData(off,'')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(skyonplus,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('-=skyonplus=- Selecione [COLOR white]Link[1][/COLOR][COLOR cyan]=720P[/COLOR] ou [COLOR white]Link[2][/COLOR][COLOR cyan]=1080P[/COLOR]', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('skyonplus Searching Please wait',' ')
...
#                    print 'item link', item('link')
                ...
                elif len(item('server01')) >0:
                    for i in item('server01'):
                        if not i.string == None:
                            server01 = 'http://cdnv2.ec.cx/RedeCanais/RedeCanais/RCServer01/videos/'+i.string+'.mp4'
                            url.append(server01)

                elif len(item('server02')) >0:
                    for i in item('server02'):
                        if not i.string == None:
                            server02 = 'http://cdnv1.ec.cx/RedeCanais/RedeCanais/RCServer02/ondemand/'+i.string+'.mp4'
                            url.append(server02)
                            
                elif len(item('server04')) >0:
                    for i in item('server04'):
                        if not i.string == None:
                            server04 = 'http://cdnv1.ec.cx/RedeCanais/RedeCanais/RCServer04/ondemand/'+i.string+'.mp4'
                            url.append(server04)

                elif len(item('server05')) >0:
                    for i in item('server05'):
                        if not i.string == None:
                            server05 = 'http://cdnv5.ec.cx/RedeCanais/RCServer05/ondemand/'+i.string+'.mp4'
                            url.append(server05)
							
                elif len(item('server06')) >0:
                    for i in item('server06'):
                        if not i.string == None:
                            server06 = 'http://cdnv1.ec.cx/RedeCanais/RedeCanais/RCServer06/ondemand/'+i.string+'.mp4'
                            url.append(server05)

                elif len(item('server07')) >0:
                    for i in item('server07'):
                        if not i.string == None:
                            server07 = 'http://cdnv1.ec.cx/RedeCanais/RedeCanais/RCServer07/ondemand/'+i.string+'.mp4'
                            url.append(server07)

                elif len(item('server08')) >0:
                    for i in item('server08'):
                        if not i.string == None:
                            server08 = 'http://vd.ec.cx/vd/dd/RC/RCServer08/ondemand/'+i.string+'.mp4'
                            url.append(server08)
                            
                elif len(item('server09')) >0:
                    for i in item('server09'):
                        if not i.string == None:
                            server09 = 'http://cdnv2.ec.cx/RedeCanais/RedeCanais/RCServer09/ondemand/'+i.string+'.mp4|'
                            url.append(server09)


                elif len(item('server10')) >0:
                    for i in item('server10'):
                        if not i.string == None:
                            server10 = 'http://cdnv4.ec.cx/RedeCanais/RedeCanais/RCServer10/ondemand/'+i.string+'.mp4'
                            url.append(server10)

                elif len(item('server11')) >0:
                    for i in item('server11'):
                        if not i.string == None:
                            server11 = 'http://cdnv4.ec.cx/RedeCanais/RedeCanais/RCServer11/ondemand/'+i.string+'.mp4'
                            url.append(server11)

                elif len(item('server12')) >0:
                    for i in item('server12'):
                        if not i.string == None:
                            server12 = 'http://cdnv3.ec.cx/RedeCanais/RCServer12/ondemand/'+i.string+'.mp4'
                            url.append(server12)
                ...
                elif len(item('kodishiptv')) >0:
                    for i in item('kodishiptv'):
                        if not i.string == None:
                            passwordl = addon.getSetting('password')
                            kodishiptv = ""+i.string+""+passwordl+""
                            url.append(kodishiptv)
                ...
                elif len(item('daily')) >0:
                    for i in item('daily'):
                        if not i.string == None:
                            daily = 'plugin://plugin.video.playthis/?mode=play&player=false&path=http://www.dailymotion.com/video/'+i.string+''
                            url.append(daily)
...
def kodish_iptv():
    workfile =  xbmc.translatePath("special://home/userdata/app_data/pvr.iptvsimple/settings.xml").decode("utf-8")
    dialog = xbmcgui.Dialog()
    token = dialog.input('[COLOR yellow]Digite o Token Kodish Iptv Depois Ative Simple Client Caso Esteja desativado[/COLOR]', type=xbmcgui.INPUT_ALPHANUM)
    kodishiptv.lista(workfile,token)
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(skyonplus,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(skyonplus,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('skyonplus','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('skyonplus', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            if ret:
                url = 'plugin://plugin.video.quasar/movie/'+imdbID+'/play'
                page_data = name +':'+ url+ '\n'
                SaveToFile(history,page_data,append=True)
                return url
        else:
            xbmc.executebuiltin("XBMC.Notification(skyonplus,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(skyonplus ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(skyonplus,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
