...
from universalscrapers import proxy
...
# someone changed all this ? get them to fix
class Primewire(Scraper):
    domains = ['primewire.ag']
    name = "primewire"

    def __init__(self):
        self.base_link = xbmcaddon.Addon('script.module.universalscrapers').getSetting("%s_baseurl" % (self.name))
                        ...
                        if any(x in link_title for x in years) or not "(" in link_title:
                            ...
                            if title.lower() == clean_title(link_title):
                                ...
                                if '(%s)' % str(year) in link_title:
                                    return self.sources(href,title,year,'','',start_time)
                                else:
                                    fallback = href
                                        ...
                                        for link in links:
                                            if link.contents[0].strip() == "E%s" % episode:
                                                ...
                                                try:
                                                    ...
                                                    pass
                                                return self.sources(episode_href,title,year,season,episode,start_time)
                except:
                    continue
            return []
        except Exception, argument:        
    ...
    def sources(self, url, title, year, season, episode, start_time):
        sources = []
        try:
